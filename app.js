...
function patientListStatusHtml(patient) {
  ...
  if (typeof hook === "function") {
    try {
      const custom = hook(patient, { completed, blockers: waits.slice() });
      if (typeof custom === "string" && custom) return custom;
    } catch (error) {
      console.error("Patient-list UI hook failed; using stable status renderer.", error);
    }
  }
  ...
  if (waits.length) {
    return `<div class="wait-stack">${waits
